...
if (isset($_POST["inserts"]) && !empty($_POST["inserts"])) {
    ...
    $a_s_course = $help->filter_data($_POST["a_s_course"]);
    $a_s_c_student = $help->filter_data($_POST["a_s_c_student"]);
    ...
    if (!isset($a_s_c_student) || empty($a_s_c_student)) {
        $status['error']++;
        array_push($status["msg"], "STUDENT IS REQUIRED");
    }

    if (!isset($a_s_course) || empty($a_s_course)) {
        $status['error']++;
        array_push($status["msg"], "COURSE IS REQUIRED");
    }



    $check = "SELECT * FROM `" . _std_course . "` WHERE `student_id`='{$a_s_c_student}' AND `course_id`='{$a_s_course}' ";
    $exe = $db->Mysql($check, true);

    if ($exe) {

        $status['error']++;
        array_push($status["msg"], "ALREADY EXIST IN THIS COURSE");
    }
    ...
    if ($status["error"] > 0) {
        ...
        $data = [
            "student_id" => $a_s_c_student,
            "course_id" => $a_s_course,
        ];

        echo $db->insert(_std_course, $data);
